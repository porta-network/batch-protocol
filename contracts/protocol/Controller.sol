...
contract Controller is Ownable {
    ...
    event SetAdded(address indexed _batchToken, address indexed _factory);
    event SetRemoved(address indexed _batchToken);
    ...
    address[] public sets;
    // List of enabled factories of BatchTokens
    address[] public factories;
    // List of enabled Modules; Modules extend the functionality of BatchTokens
    address[] public modules;
    // List of enabled Resources; Resources provide data, functionality, or
    // permissions that can be drawn upon from Module, BatchTokens or factories
    ...
    /**
     * PRIVILEGED FACTORY FUNCTION. Adds a newly deployed BatchToken as an enabled BatchToken.
     *
     * @param _batchToken               Address of the BatchToken contract to add
     */
    function addSet(address _batchToken) external onlyInitialized onlyFactory {
        require(!isSet[_batchToken], "Set already exists");

        isSet[_batchToken] = true;

        sets.push(_batchToken);

        emit SetAdded(_batchToken, msg.sender);
    }

    /**
     * PRIVILEGED GOVERNANCE FUNCTION. Allows governance to remove a Set
     *
     * @param _batchToken               Address of the BatchToken contract to remove
     */
    function removeSet(address _batchToken) external onlyInitialized onlyOwner {
        require(isSet[_batchToken], "Set does not exist");

        sets = sets.remove(_batchToken);

        isSet[_batchToken] = false;

        emit SetRemoved(_batchToken);
    }
