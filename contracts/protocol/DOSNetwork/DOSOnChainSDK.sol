...
interface DOSProxyInterface {
    function query(address, uint, string memory, string memory) external returns (uint);
    function requestRandom(address, uint) external returns (uint);
}

interface DOSPaymentInterface {
    function setPaymentMethod(address payer, address tokenAddr) external;
    function defaultTokenAddr() external returns(address);
}
...
contract DOSOnChainSDK is Ownable {
    ...
    function __callback__(uint queryId, bytes calldata result) virtual external {
